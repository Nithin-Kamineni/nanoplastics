...
const sizes = [0.02,0.22,6,1]

const url = "http://localhost:9005/model";
...
const SamplePage = () => {
    ...
    const handelBW = (event, newValue) => {
        if(newValue<=0.035 && newValue>=0.015 )
        setBW(newValue);
    }
    ...
    useEffect(()=>{
        const fetchGraphData = async () => {
            try {
                ...
                const requestBody = {"BW":BW, "scale": 220, "concentration":concentration, "treatment":treatment,"duration":duration};
        ...
        <MainCard title="Model Prediction">
            <Box display="flex" flexDirection="column" alignItems="flex-start" marginBottom="16px">
                ...
                <>
                    ...
                    <Box display="flex" justifyContent="space-around" width="100%" marginBottom="30px">
                        <Box display="flex" flexDirection="column" alignItems="center">
                            <Typography variant="h4" gutterBottom>Liver</Typography>
                            <GraphComponent2 isNano={isNano} LinePoints={graphData.kl} ScatterPoints={graphData.obskl} YaxisLabel='Y axis test'/>
                        ...
                        <Box display="flex" flexDirection="column" alignItems="center">
                            <Typography variant="h4" gutterBottom>Lung</Typography>
                            <GraphComponent2 isNano={isNano} LinePoints={graphData.klu} ScatterPoints={graphData.obsklu}  YaxisLabel='Y axis test'/>
                    ...
                    <Box display="flex" justifyContent="space-around" width="100%" marginBottom="30px">
                        <Box display="flex" flexDirection="column" alignItems="center">
                            <Typography variant="h4" gutterBottom>Spleen</Typography>
                            <GraphComponent2 isNano={isNano} LinePoints={graphData.ks} ScatterPoints={graphData.obsks} YaxisLabel='Y axis test'/>
                        ...
                        <Box display="flex" flexDirection="column" alignItems="center">
                            <Typography variant="h4" gutterBottom>Kidney</Typography>
                            <GraphComponent2 isNano={isNano} LinePoints={graphData.kk} ScatterPoints={graphData.obskk} YaxisLabel='Y axis test'/>
                ...
                <Box display="flex" justifyContent="center" width="100%">
                    ...
                    <Button variant="contained" color="secondary" onClick={defaultValues} style={{ alignSelf: 'flex-start', marginLeft:10 }}>
                        Reset
                    </Button>
                </Box>
                    ...
                    <Box id="align row wise" display="flex" flexDirection="column" alignItems="flex-start" marginBottom="16px">
                        <FormControl variant="outlined" style={{ minWidth: 400, marginTop: '16px' }}>
                            ...
                            <FormControl variant="outlined" style={{ minWidth: 200, marginTop: '16px' }}>
                                ...
                                <NumberInput value={concentration} precision={4} min={0.005} max={5} step={0.01} onChange={handelConcentration}>
                                    <NumberInputField />
                                    <NumberInputStepper>
                                        <NumberIncrementStepper />
                                        <NumberDecrementStepper />
                        ...
                        <FormControl variant="outlined" style={{ minWidth: 400, marginTop: '16px' }}>
                            ...
                            <FormControl variant="outlined" style={{ minWidth: 200, marginTop: '16px' }}>
                                <Select value={treatment} onChange={handleTreatment}>
                                    {treatmentData.map((treatment)=>(<option value={treatment.id}>{treatment.name}</option>)
                                    )}
                                </Select>
                            </FormControl>
                        </FormControl>
            ...
            <Box display="flex" justifyContent="center" marginTop={4}>
                <Button variant="contained" color="primary" onClick={()=>{setPredict(!predict)}}>
                    Run Simulation
                </Button>
            </Box>
        </MainCard>
